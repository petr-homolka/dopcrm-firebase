...
export function ChildNotesTab({ notes, hasMoreNotes, onLoadMoreNotes, noteText, setNoteText, onAddNote, submitting, submitError, canManage = true }) {
  ...
  return (
    <Card>
      <h2 className="mb-1 text-base font-semibold text-ink-800">{t('child.detail.notesHistory.notesTitle')}</h2>
      ...
      {submitError && <div className="mb-3 rounded-xl bg-danger-50 px-3.5 py-2.5 text-sm text-danger-700">{submitError}</div>}

      {notes.length === 0 && <p className="text-sm text-ink-500">{t('child.detail.notesHistory.noNotes')}</p>}
      <ul>
        {notes.map((n) => (
          <li key={n.id} className="border-t border-border-subtle py-2.5 first:border-t-0">
            <p className="text-sm text-ink-800">{n.text}</p>
            <p className="text-xs text-ink-400">{n.createdAt ? n.createdAt.toDate().toLocaleString('cs-CZ') : ''}</p>
          </li>
        ))}
      </ul>
...
export function ChildHistoryTab({ history, hasMore, onLoadMore }) {
  ...
  return (
    <Card>
      <h2 className="mb-1 text-base font-semibold text-ink-800">{t('child.detail.notesHistory.historyTitle')}</h2>
      <p className="mb-3 text-sm text-ink-500">
        {t('child.detail.notesHistory.historyDescription')}
      </p>

      {history.length === 0 && <p className="text-sm text-ink-500">{t('child.detail.notesHistory.noHistory')}</p>}
      <ul>
        {history.map((h) => (
          <li key={h.id} className="border-t border-border-subtle py-2.5 first:border-t-0">
            <p className="text-sm text-ink-800">{h.field}: {h.from} → {h.to}</p>
            <p className="text-xs text-ink-400">{h.by}</p>
          </li>
        ))}
      </ul>
