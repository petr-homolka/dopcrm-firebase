...
import { ArrowLeft, Loader2, MapPin, Phone, Mail, CalendarPlus, FileText } from 'lucide-react';
  ...
  return (
    <div>
      <div className="mb-5 flex flex-wrap items-start gap-3">
        ...
        {family && <Avatar name={family.name} size="lg" />}
        <div className="min-w-0 flex-1">
          <h1 className="break-words text-[28px] font-bold leading-tight text-ink-900">
        ...
        {family && canManage && (
          <div className="flex shrink-0 flex-wrap gap-2">
            ...
            <Button variant="primary" size="sm" onClick={() => toast.info(t('today.quickActions.reportsNotAvailable'))}>
      ...
      {!loading && !error && family && (
        <>
          <Card className="mb-5 flex flex-col gap-1.5">
            <p className="text-xs font-semibold uppercase tracking-wide text-ink-400">{t('family.detail.contactSection')}</p>
            {family.address && (
            ...
            {family.note && (
              <>
                <p className="mt-1.5 text-xs font-semibold uppercase tracking-wide text-ink-400">{t('family.detail.noteSection')}</p>
                <p className="text-sm text-ink-700">{family.note}</p>
              </>
            )}
          </Card>
