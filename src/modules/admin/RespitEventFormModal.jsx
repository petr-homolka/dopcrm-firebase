    ...
    >
      <Card className="w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-base font-semibold text-ink-800">Zaznamenat čerpání respitu</h2>
          <button
            type="button"
            onClick={() => !submitting && onClose()}
            aria-label="Zavřít"
        ...
        <form onSubmit={onSubmit} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <input
              ...
              disabled={submitting}
            />
            <p className="mt-1 text-xs text-ink-400">Prázdné = jednodenní čerpání.</p>
          </div>
          ...
          <div>
            <p className="mb-1.5 text-sm text-ink-500">Které děti byly mimo domov</p>
            <div className="flex flex-col gap-1.5">
              {childrenList.map((c) => (
                <label key={c.id} className="flex items-center gap-2.5 text-sm text-ink-700">
                  <input
                    type="checkbox"
