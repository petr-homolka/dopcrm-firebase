...
| Funkce | Popis | Kde popsáno | Stav |
|---|---|---|---|
| Design systém „Connecteam" (světlé plátno, module barvy, sidebar+topbar+right rail) | Nahrazuje „Přítomnost"/Amie styl (archiv `docs/DESIGN-amie-archiv.md`); tokeny, komponenty, layout, obrazovky, kalendář | `DESIGN.md` (redesign 2026-07-04) | 🟡 zavádí se postupně — Krok 1 hotový: `tailwind.config.js` (brand/accent/surface/border/ink/sémantika/module/entity/shift tokeny), `src/components/ui/` (Button/Badge/Avatar+double-avatar/Card/Chip/EmptyState/LoadMoreButton přebarveny, nově Input/Tabs/Table/ProgressBar/Toast/Modal/Drawer). Krok 2 hotový: `AdminLayout.jsx` přestavěn na `AdminSidebar.jsx` (240px, module dlaždice, PRACOVNÍ PROSTOR/ADMINISTRACE, mobilní drawer) + `AdminTopbar.jsx` (search rodin, zvonek/nápověda/avatar dropdown), plátno `surface-canvas`, obsah max 1280px. Sidebar zatím odkazuje jen na SKUTEČNĚ existující routy per roli (Osa/Dokumenty/Čerpání jako moduly čekají na své vlastní stránky, sekce 12). Staré `primary`/`stone` tokeny a starý `core/Layout.jsx` (Sekce A) ZÁMĚRNĚ ponechány souběžně, ať nepřevedené obrazovky (Krok 3–4) nepřijdou o styl předčasně. Krok 3a (Dnes) hotový. Krok 3b (Rodiny) hotový: `FosterFamiliesPanel.jsx` a `KlicovaOsobaDashboard.jsx` (Moje rodiny) přepsány z karet na datovou tabulku (`Table`/`Tabs`/`Input` z Kroku 1) dle §5.6/§6.2 — sloupce Rodina/Typ/(Koordinátorka)/Poslední návštěva/Status, taby Aktivní/Archivované (mapované na skutečný `status`, ne fiktivní kategorie), search rodin. `NewFamilyModal.jsx` přepsán na sdílený `Modal`+`Input`. Vědomě MIMO rozsah: onboarding cards carousel, checkbox hromadné akce, sloupec počtu dětí (vyžadovalo by denormalizaci/N+1 dotaz), ⚙ column customization, "Návrhy" tab, reálná stránkovací "1–50 z N". Krok 3c (Osa) hotový: `TimelineEntryCard.jsx` dostal double-avatar (autor + kroužek barvy modulu s ikonou typu záznamu, §5.5/§5.7 — signature vzor), `FosterFamilyTimelineTab.jsx` dostal day-chip strip nad feedem (scroll-to-day, §5.7) a filtr chipy přes sdílený `Chip`. Autor se dohledává 1 dotazem na `users` organizace (ne N+1). Ověřeno živě reálným zápisem poznámky (iniciály autora + fialový kroužek modulu Osa). Krok 3d (Detail rodiny) hotový: `FosterFamilyDetailPage.jsx` dostal header dle §6.3 (hero `Avatar` size lg + H1 28px + status badge + primární akce „Naplánovat návštěvu"/„Vyplnit report" — druhá je stub `toast.info`, sdílí i18n klíče s Dnes), manuální tab-loop nahrazen sdíleným `Tabs`. VĚDOMĚ zachována doménová struktura tabů (Osa/Pěstouni/Respit a SPVPP/Sociální prostor/Svěřené děti) místo generických příkladů z DESIGN.md (Přehled/Návštěvy/Dokumenty/Čerpání/OSPOD/Aktivita) — §1 dokumentu IA výslovně řadí mezi „Zachováno" a Dokumenty/OSPOD tab nemá zatím žádnou funkci pod sebou. Zbylé 4 tab soubory (`FosterFamilyFostersTab`/`RespitTab`/`SocialTab`/`ChildrenTab.jsx`) přebarveny na nové tokeny (mechanický průchod, žádná změna chování). Ověřeno živě na 1280px i 390×844, všech 5 tabů. Krok 3e (TeamDashboard) hotový: `TeamDashboard.jsx` přebarven na nové tokeny, H1 sjednocen na styl ostatních obrazovek (28px bold), přidán řádek 2 stat karet (Klíčové osoby v podřízenosti / Rodiny v týmu) — odvozeno z už načtených dat, žádný extra dotaz. Plný 3×2 KPI grid z §6.6 (trendy, průměrná doba do reportu, nevydané dokumenty, vzdělávání progress ring, krizové intervence) + bar chart vytížení koordinátorek + activity feed milníků VĚDOMĚ NEIMPLEMENTOVÁNO — žádná z těch metrik nemá oporu v datovém modelu (nikde se neukládají trendy, čas do reportu, krizové intervence); fabrikace čísel by byla v rozporu se zavedeným vzorem „poctivé prázdno místo fiktivních dat". Ověřeno živě na 1280px i 390×844 (prázdný stav, demo účet `demo.vedouci.jih` bez podřízených). Krok 4a (Kalendář — týdenní mřížka) hotový: nový tab „Týden" vedle stávající agendy (`CalendarWeekGrid.jsx` + `useCalendarWeek.js`) — koordinátorky (role klíčová osoba) jako řádky, dny jako sloupce, bloky návštěv barevné podle typu události (`shift.*` paleta z Kroku 1, namapováno na skutečné `EVENT_TYPES` — DESIGN.md §2.4 bohatší taxonomii OSPOD/Soud/bio-rodina/krize/metodika v datovém modelu nemá, viz komentář v `calendarShared.js`). Řádek „Bez přiřazení" pro události bez shodujícího se `assignedTo`. `EventFormModal.jsx` přepsán na sdílený `Modal`/`Input`. Objevena a opravena souvisejicí chyba: route `/kalendar` byla zaregistrovaná jen ve staré Sekci A (`RequireAuth`/starý `Layout.jsx`), takže i po Kroku 2 se kalendář otevíral MIMO nový `AdminSidebar`/`AdminTopbar` shell pro všechny role — přesunuto do vlastního `AdminLayout` bloku v `router.jsx` (`RequireOrgRole` pro klicova_osoba/org_admin/vedouci_pobocky/teamleader/superadmin), ověřeno pro klicova_osoba i vedouci_pobocky. Capacity bary/sticky footer (4b), publish workflow (4c) a šablony/otevřené návštěvy (4d) zatím NEIMPLEMENTOVÁNO. Krok 4b (sticky footer) hotový: mřížka dostala 8. sloupec „Týden" (součet za koordinátorku i za celou organizaci) a patičkové řádky Návštěvy/Rodiny (počet návštěv a UNIKÁTNÍCH rodin za den + týdenní součet jako sjednocení, ne prostý součet dnů). Řádek „Hodiny" a capacity bary „2/3" pod day headery z DESIGN.md §6.4 VĚDOMĚ NEIMPLEMENTOVÁNY — události v datovém modelu nemají délku trvání (jen `start`, žádný `end`) a koordinátorky nemají nastavenou denní kapacitu; obojí by muselo být fabrikované číslo bez opory v datech. Krok 4c (publish workflow) hotový: událost dostala reálné pole `published` (events.js), `EventFormModal.jsx` checkbox „Uložit jako koncept", koncepty mají šrafovaný overlay dle §2.4, tlačítko „Publikovat (N)" (jen když existují koncepty, které smí publikovat AKTUÁLNÍ uživatel — `canPublishEvent()` zrcadlí firestore.rules: klíčová osoba jen svoje, management celou organizaci) otevře `PublishModal.jsx` a publikace je skutečný `writeBatch` zápis (`publishEvents()` v events.js). Checkbox „Odeslat oznámení" je VĚDOMĚ jen vizuální stub (toast) — appka nemá push/e-mail infrastrukturu pro koordinátorky. Diff „N nových/M upravených/K zrušených" z DESIGN.md zjednodušen na jedno číslo (nesledujeme historii změn konceptu). Ověřeno živě: založení konceptu, šrafování, publikace, zmizení šrafování i tlačítka po publikaci — 1280px i 390×844. Krok 4d (šablony + otevřené návštěvy) hotový — a tím i celý Kalendář (Krok 4a–4d): `EventFormModal.jsx` dostal 3 šablony ze specifikace (Standardní návštěva/První kontakt/Krizová intervence) jako rychlé předvyplnění názvu+typu (klientská pohodlnost, žádná perzistence šablon v Firestore). Pro management (org_admin/vedouci_pobocky/teamleader/superadmin) checkbox „Otevřená návštěva" zakládá událost s `assignedTo: null`; taková událost dostane v mřížce přerušovaný rámeček a text „Otevřená návštěva" místo barvy typu (§6.4). VĚDOMĚ NEIMPLEMENTOVÁNO: tlačítko „Přijmout" pro klíčovou osobu — firestore.rules `update` na `events` vyžaduje `resource.data.assignedTo == request.auth.uid`, takže klíčová osoba si nepřiřazenou událost sama převzít nemůže; vyžadovalo by to změnu pravidel (bezpečnostní rozhodnutí nad rámec vizuálního redesignu). Drag&drop (přesun bloku mezi buňkami/koordinátorkami), Requests tab a Users tab (postranní panel s weekly hours) z DESIGN.md §6.4 také NEIMPLEMENTOVÁNY. Ověřeno živě jako klíčová osoba (šablony, bez checkboxu otevřené návštěvy) i jako vedoucí pobočky (checkbox funguje, otevřená návštěva viditelná v mřížce) — 1280px i 390×844. **Mobilní PWA dle reálných Connecteam screenshotů (2026-07-04, §11):** uživatel dodal 56 skutečných screenshotů Connecteam mobilní appky → nová sekce DESIGN.md §11. Navigační kostra hotová: `MobileTabBar.jsx` (spodní tab bar per roli, 2–4 ikony) nahradil hamburger+`AdminSidebar` mobilní drawer; `AdminSidebar.jsx` teď čistě `hidden lg:block`, `AdminTopbar.jsx` čistě `hidden lg:flex` (na mobilu appka nemá sdílený topbar vůbec — každá obrazovka má vlastní hlavičku). Nová `MobileProfilePage.jsx` (route `/profil`, všechny Sekce B role) — hero karta s dekorativním pastelovým pozadím + HRANATÝ avatar (jediné místo v appce, jinak vždy kulatý `Avatar.jsx`) + list Nastavení/Odhlásit se, nahrazuje avatar dropdown z topbaru na mobilu. `roleLabel()` extrahováno do `orgAuth.js` (dřív duplicitní `ROLE_LABELS` jen v AdminTopbar). Desktop beze změny (screeny pro desktop redesign dodá uživatel později). Dnes (mobil) hotovo: hlavička dostala avatar vedle pozdravu (funguje i na desktopu, bonus), quick actions jsou na mobilu 2×2 pill dlaždice (`TodayQuickActions.jsx`, vytaženo z `TodayPage.jsx` pro limit 300 řádků) — jen „Naplánovat návštěvu" barevná (skutečně funkční), zbylé 3 neutrální (toast stuby). Zvonek s notifikacemi z §11.2 VĚDOMĚ vynechán (appka nemá reálná oznámení). Rodiny (mobil) hotovo: nová `FamilyListRow.jsx` (barevný kruhový avatar + jméno/typ+poslední návštěva + status badge, §11.3) nahrazuje na mobilu (<lg) horizontálně scrollovatelnou `Table` v `FosterFamiliesPanel.jsx` i `KlicovaOsobaDashboard.jsx` (Moje rodiny) — sdíleno mezi oběma, desktop Table beze změny. Kalendář agenda (mobil) hotovo: nová `CalendarAgendaList.jsx` (vytaženo z `CalendarPage.jsx` pro limit 300 řádků) — na mobilu (<lg) shift-card vzor (§11.4, barevný levý pruh dle typu události), na desktopu beze změny z Kroku 4a. Barva pruhu (`EVENT_BORDER_CLASS`) přesunuta z `TodayPage.jsx` do `shared/domainConstants.js`, ať Dnes i Kalendář používají STEJNÉ barvy pro stejný typ události (dřív duplicitní mapa jen v TodayPage). Pill tlačítka (mobil) hotovo: `Button.jsx` (sdílená komponenta, VŠECHNY obrazovky) dostal `rounded-full lg:rounded-lg` — na mobilu plná kapsle dle §11.5, na desktopu beze změny (`rounded-lg`). Systémová změna na úrovni tokenu/komponenty, ne per-obrazovka výjimka, aby tlačítka nepůsobila nekonzistentně mezi obrazovkami. Detail rodiny (mobil) ověřen — hlavička s hero avatarem a pill akcemi (Naplánovat návštěvu/Vyplnit report) vypadá dle Connecteam vzoru beze další úpravy nad rámec Button.jsx. |
...
| Obrazovka Dnes (agenda, domovská pro klicova_osoba) | Pozdrav+datum, quick actions, „Dnes k vyřešení", dnešní program, „Čeká na vás" (lastVisitAt > 45 dní), nejbližší 3 dny, right rail se statistikou týdne; žádné KPI/grafy | `DESIGN.md` §6.1 (Connecteam redesign Krok 3a, 2026-07-04) | ✅ route `/`, `TodayPage.jsx`. 2 ze 4 quick actions zatím jen informační toast (Poslat oznámení/Vyplnit report — funkce neexistují, sekce 12); "Přidat rodinu" toast taky (klicova_osoba nemá dosud UI pro založení rodiny, i když rules by to dovolily); "Naplánovat návštěvu" reálně vede na `/kalendar`. "Dnes k vyřešení" je zatím vždy empty-state (dokumenty s expirací/reporty/OSPOD zprávy neexistují). Bez emoji (CLAUDE.md má přednost před DESIGN.md mockupy) — lucide-react ikony místo 👋📅📢📄. |
